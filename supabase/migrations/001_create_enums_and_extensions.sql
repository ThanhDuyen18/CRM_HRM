...
-- ============================================================================
-- Create custom ENUM types (using DO block to handle existence check)
-- ============================================================================
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'app_role') THEN
        CREATE TYPE app_role AS ENUM (
            'admin', 'hr', 'leader', 'teacher', 'it', 'content', 'design', 'staff'
        );
    END IF;
END
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'color_type') THEN
        CREATE TYPE color_type AS ENUM (
            'blue', 'red', 'yellow', 'green', 'purple', 'pink', 'gray', 'orange', 'cyan'
        );
    END IF;
END
$$ LANGUAGE plpgsql;
